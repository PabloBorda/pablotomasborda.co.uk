function get_possible_location(){
	 var dir = "http://soa1.getstuffclose.com:9494/possible_location?lat=" + window.global['lat'] + "&lng=" + window.global['lng'];
	 var a = "";
     $.ajax({
       url: dir,
       dataType: 'json',
...
function verify_data(){
	...
	if (($("#name").val()=="i.e: Red Hat with green flowers") || $("#name").val().length<4){
	  ...
	  if (($("#category").val()=="i.e: Clothes") || $("#category").val().length<4){
	    ...
	    if (($("#price").val()=="0") || $("#price").val().length<4){
		  validation_result = false;
...
function publish(){
	...
	if (verify_data()){
      ...
      for (window["current_file"]=0;window["current_file"]<window.piccount;window["current_file"]++){
          ...
          window.plugins.fileUploader.uploadByUri("http://soa1.getstuffclose.com:9494/new_product_social",window.pics[window["current_file"]],window.global,"pic" + window["current_file"].toString(),"pic" + window["current_file"].toString() + ".jpg","image/jpeg",success,failed);        
      }
